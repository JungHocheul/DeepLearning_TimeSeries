...
with open('HomeF-meter2_2015_test3.csv') as f:
...
with open('HomeF-meter2_2015_train.csv') as f:
    reader = csv.DictReader(f)
    for row in reader :
        for (k,v) in row. items() :
...
X = X.reshape((X.shape[0],X.shape[1], n_features))
 #define model
model = Sequential()
model.add(Conv1D(filters=3, kernel_size=3, activation='relu', input_shape=(n_steps, n_features)))
...
MAPE = mean_absolute_percentage_error(test_data, yhat)
print('MAPE =', MAPE)
